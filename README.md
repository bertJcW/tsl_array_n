...
TSL 已经能写 GPU 计算（`Fn` + `.compute()` + `instancedArray`），但原生写法需要不少样板代码：手动建 renderer、手动管理 storage buffer 下标、手动做 CPU↔GPU 数据搬运。Taichi Lang 在 Python 里把这些都藏了起来——`ti.field` 声明数据，`@ti.kernel` 写并行逻辑，最外层 `for` 循环自动并行。tsl_array_n 想把这套体验搬到 three.js/TSL 上，作为一个**通用**的并行计算基础库。

## 概念对照

| Taichi | 原生 TSL | tsl_array_n |
|---|---|---|
...
| （无对应） | — | `array2(type, w, h)` / `array3(type, w, h, d)` ✅ 已实现，tsl_array_n 独有的糖，等价于对应形状的 `arrayN` |
...
推荐用 `import * as tsl_array_n` 这种命名空间写法调用（跟 `import * as THREE from 'three'` 一致），避免和其它包的同名导出（`init`、`field` 之类很通用的名字）冲突：

```js
import * as tsl_array_n from 'tsl_array_n';

const renderer = await tsl_array_n.init(); // 检测 navigator.gpu、创建 WebGPURenderer、挂到 document.body
```
...
import { init } from 'tsl_array_n';         // init()（更容易和别的包撞名，不太推荐）
```
...
```js
const scalar = tsl_array_n.array0( 'vec2' );               // 等价于 arrayN('vec2', [])，0 维，单个元素
const grid   = tsl_array_n.array2( 'float', 128, 128 );     // 等价于 arrayN('float', [128, 128])
const cloud  = tsl_array_n.arrayN( 'vec3', 4096 );          // 1D，4096 个 vec3
const volume = tsl_array_n.array3( 'float', 32, 32, 32 );   // 32³ 体素
```
...
```js
const c = tsl_array_n.uniform( new THREE.Vector2( 0.7885, 0 ) ); // 或 tsl_array_n.array0('vec2')，效果等价
...
`func` 就是 TSL 的 `Fn`，重导出成 Taichi 风格的名字而已（`tsl_array_n.func === Fn`，连包装函数都没有），用来写可以被 kernel 内部调用的设备侧小函数：
...
const prefixSum = tsl_array_n.kernel( n, ( idx ) => {
	...
	tsl_array_n.Loop( idx.add( 1 ), ( { i } ) => {   // 循环次数可以是逐线程不同的 node（这里是 idx+1）
...
`Loop(count, ({i}) => {...})` 的回调也是"单个 destructure 参数"的约定（这次是对象 `{i}`，不是 `func`/`Fn` 那种数组），嵌套循环写 `Loop(n, m, ({i, j}) => {...})`。`If(condition, fn)` 配合 `Break()`/`Continue()` 做条件跳出/跳过，用法和原生 TSL 一样，`tsl_array_n` 没有加任何东西。

> **已在真实 WebGPU 验证**：`examples/03-loop/` 在真实 `WebGPUBackend` 上跑，前缀和、`Break`/`Continue`/`If`、最小复现全部正确（`[1,3,6,10,15]` 等）。开发这个库用的 sandbox 环境（没有真实 WebGPU 适配器，`init()` 会 fallback 到 `WebGLBackend`）曾经在这个 fallback 后端上实测出错误结果——排查后确认不是 tsl_array_n 的问题（不经过 tsl_array_n、纯原生 `three/tsl` 的最小复现在那个环境下同样出错），而是那个 WebGL2 fallback 后端自己在 `Loop()` + storage buffer 交互上的限制。**结论：`Loop()`/`Break()`/`Continue()`/`If()` 需要真实 WebGPU，`allowFallback: true` 下的 WebGL2 fallback 不可信**（这跟"WebGL2 fallback 不支持 compute shader"这条已知限制是一致的，之前 `kernel()` 的简单场景之所以能在 fallback 上跑通，只是巧合地没踩到这个坑）。

> **`array0` 多线程共读也已在真实 WebGPU 验证**：`examples/04-julia/`（动画版，`c` 用 `array0('vec2')` 每帧更新，512×512 kernel 里每个线程都读同一个 `array0` 元素）在真机上动画连续正确变形。开发 sandbox 的 WebGL2 fallback 上曾经在这个"多线程并发读同一个 array0 元素 + 在 Loop() 内部读"的组合下全部读到 0——和上面 `Loop()` 那条一样，确认是 fallback 后端自己的限制，不是 tsl_array_n 的问题。
