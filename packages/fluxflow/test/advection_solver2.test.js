...
describe( 'createSemiLagrangianAdvectionSolver2', () => {
	...
	describe( 'order: 2 (MacCormack)', () => {

		it( 'constructs without throwing', () => {

			const velocityGrid = createFaceCenteredGrid2( 4, 4, 1, 1, 0, 0 );

			expect( () => createSemiLagrangianAdvectionSolver2( { velocityGrid, dt: 0.1, order: 2 } ) ).not.toThrow();
		...
		it( 'advectScalar2 returns a 0-arg dispatcher without throwing', () => {
			...
			const density = createScalarGrid2( 4, 4, 1, 1, 0, 0 );
			const output = createScalarGrid2( 4, 4, 1, 1, 0, 0 );
			const solver = createSemiLagrangianAdvectionSolver2( { velocityGrid, dt: 0.1, order: 2 } );
		...
		it( 'works together with a collider', () => {

			const velocityGrid = createFaceCenteredGrid2( 4, 4, 1, 1, 0, 0 );
			const collider = createSDFStaticCollider2( 4, 4, 1, 1, 0, 0 );
			const output = createFaceCenteredGrid2( 4, 4, 1, 1, 0, 0 );
			const solver = createSemiLagrangianAdvectionSolver2( { velocityGrid, collider, dt: 0.1, order: 2 } );

			expect( () => solver.advectFaceCentered2( velocityGrid, output ) ).not.toThrow();
