...
- [`docs/perf-investigation-cg-gpu-resident-alpha-beta.md`](docs/perf-investigation-cg-gpu-resident-alpha-beta.md)
  -- the full performance record, including every rejected direction and
  two retracted rounds of measurement.
- [`docs/openfoam-two-phase-flow.md`](docs/openfoam-two-phase-flow.md) --
...
| `grid_two_phase_flip_solver2.js` | (no counterpart -- ported from mantaflow instead, see below) | `createGridTwoPhaseFlipSolver2` -- a two-phase (liquid + gas) FLIP solver in which the air is a simulated phase rather than a `p = 0` void, coupled through a variable-density pressure projection (the `faceWeights` support below); `computeTwoPhaseBoxSeed` -- seeds a box of liquid particles into a gas-filled domain |
| `surface_tension2.js` | (no counterpart -- original code, see the file's own header comment for the model and its sources) | `createSurfaceTension2` -- surface tension on a level set, folded into the external-force stage; `examples/29-static-droplet/` is its reference scene. The FLIP solvers do not use it -- see the two-phase section's own list of known limits |
| `dye_field2.js` | (no counterpart -- original code) | `createDyeField2`/`computeDyeSeed` -- a higher-resolution passive dye field, **committed but deliberately not exported**: parked pending the agreed ordering (surface tension, then performance, then dye). See Open items |
...
elsewhere in this port for this exact class of risk (`isDegenerateDenominator`, `bilinearGradientAtPosition2`'s
...
bleeds across the interface); no level set, so no sub-cell ghost-fluid interface and no surface tension *in this solver* -- a surface-tension model has since landed separately as `surface_tension2.js` for a level-set scene (`examples/29-static-droplet/`), and this solver still neither has a level set nor uses it;
...
> **Renamed from `isDegenerateDot`, and its signature changed** (2026-09-13). The
> old form took a `scale` and bailed out on anything below `0.5/scale`, which was
> correct while the dot product was a fixed-point atomic accumulation and wrong
> afterwards -- a converging CG falls through that floor as a matter of course. The
> current form takes the denominator alone and treats only an exact 0 or a
> non-finite value as degenerate; the magnitude plausibility check moved to the
> quotient (`MAX_ALPHA_MAGNITUDE`). The prose below is the original investigation
