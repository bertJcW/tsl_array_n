...
- **`vectorGradient2`/`vectorGradientAtPosition2`'s mat2 element order hasn't been numerically verified on real hardware yet** (whether Taichi's `tm.mat2` and TSL's `mat2` fill in the same order, row-major vs column-major -- a direct translation could end up transposed).
...
## Known limitations (sandbox environment constraints, not a code problem with this port)

Both `examples/00-grid-math/` (`grid_math.js` numeric verification) and `examples/01-boundary-condition/` (boundary-condition-solver dispatch smoke test) have been run in this dev sandbox (no real WebGPU adapter, `init()` falls back to `WebGLBackend`):

- `01-boundary-condition/` is all green -- the full pipeline (collider rasterization -> constructing the boundary solver -> `constrainVelocity()` genuinely dispatching a whole set of kernels -> switching via `setCollider(null,...)`) runs end to end without throwing, which has value on its own (it catches API misuse/wrong argument counts/misspelled method names), but **the numbers read back are not a correctness check** (see the next point).
- All three numeric tests in `00-grid-math/` read back 0 -- investigated (see the header comment in `examples/00-grid-math/main.js`): a single-thread kernel assigning a constant writes back correctly, so the problem is specifically that "reading a different field that already has data, from inside a kernel" doesn't read back the right value on this fallback backend, regardless of whether that data was written via `fromArray()` or by another kernel. This is the third time tsl_array_n has hit "the sandbox's WebGL2 fallback gives wrong results in multi-kernel scenarios, confirmed fine on real WebGPU" (the previous two were the `Loop()` counter and `array0` multi-thread shared reads) -- probably the same class of environment limitation, but **this one hasn't been confirmed on real WebGPU hardware yet**.

**`grid_math.js`'s numeric correctness (including the mat2 ordering question) and `grid_blocked_boundary_condition_solver2.js`'s actual physical behavior are both still unconfirmed -- both examples need to be run in a real WebGPU environment to confirm them.**
