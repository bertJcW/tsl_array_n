// *** SANDBOX COPY -- not part of the library ***
//
// A duplicate of src/linalg/multigrid.js with one change: the smoother is
// damped Jacobi with ping-pong buffers instead of red-black Gauss-Seidel.
// It lives here rather than behind a flag in src/ because the change is
// structural -- Jacobi cannot update in place, so every level needs a
// second x -- and the point is to find out whether it is worth that before
// the real file pays for it.
//
// Compared against the real one by examples/30-jacobi-smoother-sandbox/,
// which builds both from the same operator and the same CG solver so the
...
import { buildElementwiseKernel } from '../../src/linalg/linalg.js';
import { instrumentDispatch, profileBatch } from '../../src/profiling.js';
...
// One damped-Jacobi sweep. Reads every value from `xRead` and writes every
// value to `xWrite`, which is why it needs two buffers: a Jacobi sweep is
// defined on the *old* iterate, so writing into the array being read is a
// race. Red-black avoids that by writing only one colour per dispatch,
// whose neighbours are all the other colour -- it buys in-place updating
// with a second dispatch.
//
// `omega` is the damping factor and it matters enormously: on a float64
// reference of this operator, 2/3 converged in 29 iterations and 0.8 in
// 31, against red-black's 30 -- but undamped (omega = 1) took 204.
function buildJacobiRelaxKernel( shape, spacing, omega, xRead, xWrite, b, dirichletMask, faceWeights ) {

	return buildElementwiseKernel( shape, ( I ) => {

		const Ax = laplacianAt( xRead, spacing, shape, I, dirichletMask, faceWeights );
		const diagonal = laplacianDiagonalAt( spacing, shape, I, dirichletMask, faceWeights );
		const current = xRead( ...I );

		xWrite( ...I ).assign( current.add( b( ...I ).sub( Ax ).div( diagonal ).mul( omega ) ) );
...
export function createMultigridPreconditioner( shape, gridSpacing, options = {} ) {
	...
	function applyMultigridPreconditioner( input, output ) {
		...
		for ( let level = 0; level < numberOfLevels; level ++ ) {
			...
			// The ping-pong partner for x. Allocated at every level,
			// including level 0 whose x is the caller's output field --
			// the sweeps still land back in that one, so the caller never
			// sees this.
			const xAlt = tsl_array_n.arrayN( 'float', levelShape );
			...
			levels.push( {
				...
				// The ping-pong pair. Every sweep count in this file is
				// even (2 down, 2 up, 2 final, 20 coarsest), so a run of
				// sweeps always lands back in `x` and nothing downstream --
				// residual, restrict, correct -- has to know this exists.
				jacobiForward: buildJacobiRelaxKernel( levelShape, levelSpacing, omega, x, xAlt, b, levelMask, levelFaceWeights ),
				jacobiBackward: buildJacobiRelaxKernel( levelShape, levelSpacing, omega, xAlt, x, b, levelMask, levelFaceWeights ),
		...
		// *** The one change this sandbox exists for ***
		//
		// `reversed` is accepted and ignored. Red-black needed it because a
		// V-cycle is a symmetric operator only if post-smoothing undoes
		// pre-smoothing's colour order (see the long note this file
		// inherits at the original relax). A Jacobi sweep has no colour
		// order to undo -- it is symmetric by construction -- which is the
		// second reason to want it, independent of the dispatch count.
		//
		// Each sweep is ONE dispatch instead of two, alternating between
		// the buffers. Every call site here passes an even count, so the
		// result is always back in `x` when this returns.
		function relax( queue, level, iterations, reversed = false ) {

			if ( iterations % 2 !== 0 ) {

				throw new Error( `multigrid (jacobi sandbox): sweep counts must be even so the ping-pong lands back in x, got ${ iterations }.` );

			}

			for ( let i = 0; i < iterations; i ++ ) {

				queue.push( i % 2 === 0 ? levels[ level ].jacobiForward : levels[ level ].jacobiBackward );
		...
		function vCycle( queue, level, useSingleGroupCoarse, foldClear ) {
			...
			if ( level === numberOfLevels - 1 ) {
				...
				// The single-workgroup coarse kernel is a red-black
				// construction (barriers between colours) and has no
				// Jacobi counterpart here, so this sandbox always takes
				// the per-sweep path. The comparison therefore runs with
				// coarseSingleGroup off in BOTH arms -- see the example.
				const singleGroup = null;
